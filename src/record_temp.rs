use std::{fs::{File, read_dir}, io::Error, ops::Add, path::Path, usize};
...
pub fn action(outdir: &str) {
    ...
    let file = create_record_file(outdir,None)
    .expect("Fail Create File");

    

    let mut addedOneRecording = false;
    loop {
        ...
        {
            let data = record_mouse(&device_state);
            continue;
        }
...
fn create_record_file(outdir: &str, filename: Option<&str>) -> Result<File,Error> {
    ...
    let filename = match filename {
        ...
        None => {
            ...
            let newfile = [
                "recording",
                current_file_count.add(1).to_string().as_ref()
                ]
                .join("_");
...
fn count_total_recordings(outdir: &str)-> Result<usize, Error>{
    let a = read_dir(outdir)?.count();
